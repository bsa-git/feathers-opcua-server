...
async function onChangedKepValue(params, dataValue) {
  ...
  try {
    ...
    const ownerGroupName = addressSpaceOption.ownerGroup;
    ...
    // Get subscribeParams
    const ownerGroupValue = allVariables.find(v => v.browseName === ownerGroupName);
    const subscribeParams = ownerGroupValue.subscribeParams;

    if(subscribeParams.srvEndpointUrl){
      console.log('onChangedKepValue.subscribeParams:', subscribeParams);
    }
